...
void Triangle::InRnd(FILE *input_rnd) {
    do {
        x1 = Shape::rnd1000.Get();
        y1 = Shape::rnd1000.Get();
        x2 = Shape::rnd1000.Get();
        y2 = Shape::rnd1000.Get();
        x3 = Shape::rnd1000.Get();
        y3 = Shape::rnd1000.Get();
    } while (Side(x1, y1, x2, y2) >= Side(x1, y1, x3, y3) + Side(x2, y2, x3, y3) ||
    Side(x1, y1, x3, y3) >= Side(x1, y1, x2, y2) + Side(x2, y2, x3, y3) ||
    Side(x2, y2, x3, y3) >= Side(x1, y1, x2, y2) + Side(x1, y1, x3, y3));
    ...
    switch (index) {
        ...
        case 2: {
            color = Triangle::Color::ORANGE;
            break;
        }
        case 3: {
            color = Triangle::Color::YELLOW;
            break;
        }
        case 4: {
            color = Triangle::Color::GREEN;
            break;
        }
        case 5: {
            color = Triangle::Color::LIGHT_BLUE;
            break;
        }
        case 6: {
    ...
    fprintf(input_rnd, "TRIANGLE %d %d %d %d %d %d %s \n", x1, y1, x2, y2, x3, y3, ColorToString(color).c_str());
}
